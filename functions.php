...
function vicuna_tag_cloud( $args = '' ) {
	...
	$return = "<ul class=\"tagCloud clearfix\">\n\t";
	$return .= join("\n\t", $a);
	$return .= "\n</ul>\n";
...
function vicuna_paging_link($args = '') {
	...
	$defaults = array('next_label' => __('新しい記事'), 'prev_label' => __('前の記事'), 'indent' => '');
...
function vicuna_edit_comments_link($link = 'Edit This Comments.', $before = '', $after = '') {
	...
	set_post_thumbnail_size( 650, 500, true ); // 通常の投稿サムネイル
	add_image_size( 'single-post-thumbnail', 200, 170 ); // 個別投稿、個別の固定ページでのサムネイル
}
